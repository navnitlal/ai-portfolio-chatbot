...
Goal: Ensure the assistant does NOT claim to have performed actions or used data sources unless the application actually did so.
Also ensure the assistant does NOT state portfolio facts unless backed by available stored data.
...
- If the user asked for metrics/trends but prerequisites are missing (e.g. no data for metrics), respond by asking for what's needed.
...
def enforce_policy(
    ...
    payload = {
        ...
        "execution_flags": flags.__dict__,
        "issues": issues,
    }

    msg = policy_llm.invoke([HumanMessage(content=POLICY_PROMPT + "\n\nINPUT:\n" + str(payload))])
